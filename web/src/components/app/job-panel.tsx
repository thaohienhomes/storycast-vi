...
function stageItems(job: Job, events: JobEvent[], t: T): TodoItem[] {
  ...
  return STAGES.map(([key, label], i) => {
    ...
    return {
      ...
      title: t(label),
      status,
      detail: status === "in-progress" && last ? <span className="block max-w-[11rem] truncate sm:max-w-[13rem]">{t(last.msg)}</span> : undefined,
    };
  });
}
    ...
    <>
      {!planned && (
        <div className="rounded-2xl border border-border bg-background/50 p-4">
          <TextShimmer className="text-xs">{t("The director is writing the script…")}</TextShimmer>
      ...
      <div className="rounded-2xl border border-dashed border-border-strong bg-background/30 p-2">
        ...
        <p className="px-1 pt-2 text-xs text-muted-foreground">
          {stage === "character" ? t("Designing the character…") : stage === "keyframes" ? t("Painting the keyframes…") : t("Character sheet and keyframes will appear here")}
        </p>
      </div>
    </>
  );
...
export function JobPanel({ job, events, onResume }: Props) {
  ...
  const items = useMemo<LightboxItem[]>(() => {
    ...
    for (const e of media) {
      e.images?.forEach((u) => out.push({ url: u, kind: "image", caption: t(e.msg) }));
      e.videos?.forEach((u) => out.push({ url: u, kind: "video", caption: t(e.msg) }));
    }
    return out;
  }, [media, job.status, job.result, t]);
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">{running ? t("In production") : job.status === "done" ? t("Latest film") : t("Stopped")}</p>
          <h2 className="mt-1 truncate text-2xl font-medium tracking-tight">{plan ? `${plan.title} ${plan.subtitle}` : job.topic}</h2>
        </div>
        <AgentProgress
          label={job.status === "done" ? t("Finished") : job.status === "error" ? t("Stopped") : t(STAGES.find(([k]) => k === job.stage)?.[1] ?? "Queued")}
        ...
        {job.status === "done" && job.result && (
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">{t("{n} s · stored on fal", { n: job.result.duration })}</p>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <Download className="size-4" /> {saving ? t("Saving…") : t("Download")}
                </Button>
                <Button size="md" onClick={() => navigate(`/films/${job.id}`)}>
                  <Share2 className="size-4" /> {SHARING ? t("Watch & share") : t("Open the film page")}
                </Button>
              </div>
            </div>
          </motion.div>
      ...
      {job.status === "error" && (
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
          <p className="min-w-0 flex-1">{job.error ? t(job.error) : last ? t(last.msg) : null}</p>
          {job.resumable && onResume && (
            <Button size="sm" variant="outline" onClick={onResume}>
              {t("Resume from the last step")}
            </Button>
          )}
        </div>
      )}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,320px)_1fr]">
        <div className="flex min-w-0 flex-col gap-4">
          <TodoList items={stageItems(job, events, t)} title={t("Pipeline")} collapseOnComplete={false} maxHeight={420} />
          <div ref={logRef} className="max-h-64 overflow-y-auto rounded-2xl border border-border bg-background/50 p-3 font-mono text-[11.5px] leading-relaxed">
            {events.map((e, i) => (
              <div key={i} className={cn("flex gap-2", e.stage === "error" ? "text-destructive" : "text-muted-foreground")}>
                ...
                {i === events.length - 1 && running ? <TextShimmer className="min-w-0">{t(e.msg)}</TextShimmer> : <span className="min-w-0">{t(e.msg)}</span>}
              </div>
            ))}
          </div>
        </div>
          ...
          {plan && (
            ...
            >
              <p className="text-xs text-muted-foreground">
                {t("Script · narrated by")} <span className="text-foreground">{plan.character.name}</span>
              </p>
            ...
            {media.map((e, i) => (
              ...
              >
                <div className={cn("grid gap-1.5", (e.images?.length ?? 0) + (e.videos?.length ?? 0) > 2 ? "grid-cols-3 sm:grid-cols-4" : "grid-cols-2")}>
                  {e.images?.map((u) => (
                    <button key={u} type="button" onClick={() => openUrl(u)} className="overflow-hidden rounded-lg bg-muted" aria-label={t("Preview image")}>
                      <img src={u} alt="" loading="lazy" className="aspect-video w-full object-cover transition-transform duration-300 hover:scale-105" />
                    </button>
                  ))}
                  {e.videos?.map((u) => (
                    <button key={u} type="button" onClick={() => openUrl(u)} className="group relative overflow-hidden rounded-lg bg-black" aria-label={t("Preview shot")}>
                ...
                <figcaption className="px-1 pt-2 text-xs text-muted-foreground">{t(e.msg)}</figcaption>
              </motion.figure>
            ))}
          </AnimatePresence>
        </div>
